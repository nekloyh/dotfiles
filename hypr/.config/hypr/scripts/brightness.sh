...
step=5  # % mỗi lần — dùng chung cho keybind XF86 lẫn waybar scroll (linear, khớp % hiển thị của waybar backlight)
DEV="intel_backlight"   # actual screen backlight (nvidia_0 is dGPU dummy)

# Get current brightness as an integer (without %)
get_brightness() {
    brightnessctl --device="$DEV" -m | cut -d, -f4 | tr -d '%'
}
